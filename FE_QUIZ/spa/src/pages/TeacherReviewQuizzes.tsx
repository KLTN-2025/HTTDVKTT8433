...
const TeacherReviewQuizzes: React.FC = () => {
  const { user } = useAuth();
  ...
  if (loading) {
    return (
      <RoleGuard user={user} requireRoles={['ROLE_TEACHER', 'ROLE_ADMIN'] as UserRole[]}>
                    ...
                    <div className="absolute -inset-1 bg-gradient-to-r from-amber-300 to-orange-300 rounded-2xl opacity-0 group-hover:opacity-20 blur-xl transition-all duration-500"></div>
                      <div className="relative">
                        ...
                        <div className="flex space-x-3">
                          <Link
                            to={`/teacher/quiz-taking?quizId=${quiz.baseQuizId}&type=review&reviewQuizId=${quiz.id}`}
...
export default TeacherReviewQuizzes;
