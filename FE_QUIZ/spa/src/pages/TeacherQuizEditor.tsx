...
export default function TeacherQuizEditor() {
  ...
  const loadQuizData = async () => {
    ...
    try {
      ...
      const allAnswers = quizData.questions.flatMap(q => (q as any).answers || [])
  ...
  const handleForkQuiz = async () => {
    ...
    try {
      ...
      navigate(`/teacher/quiz/${forkedQuiz.id}/edit`)
    } catch (error) {
                              ...
                              {questionAnswers.length === 0 ? (
                                ...
                                <div className="space-y-3">
                                  {questionAnswers.map((answer, answerIndex) => {
                                    console.log('Rendering answer:', { id: answer.id, content: answer.content, isCorrect: answer.isCorrect })
                                    return (
